...
export function Modal({ open, onClose, title, children, footer, maxWidth = 420 }: ModalProps) {
  ...
  return createPortal(
    <div
      className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-scrim/50 animate-fade-in"
      onClick={onClose}
    ...
    >
      <div
        ...
        aria-modal="true"
        aria-label={title}
        className="w-full bg-surface-c rounded-3xl shadow-elevation-4 animate-fade-in overflow-hidden"
        style={{ maxWidth }}
      >
